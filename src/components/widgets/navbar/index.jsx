...
export default function NavBar() {
  ...
  return (
    <ul className="menu-container">
      <li
        className="menu-container-li"
        onMouseLeave={() => setMenuProducts(false)}
      >
        <NavLink to="#" onMouseOver={openNavProducts} onClick={openNavProducts}>
          Productos
        </NavLink>
        {windowWidth > 1000 ? (
          menuProducts ? (
            <ul className="menu-container-li-ul">
              {linkProducts.map((p) => (
                <li className="menu-container-li-ul-li" key={p.id}>
                  <NavLink to={p.url}>{p.nombre}</NavLink>
                </li>
              ))}
            </ul>
          ) : (
      ...
      >
        <NavLink
          className=""
          to="/services"
          onMouseOver={openNavServices}
          onClick={openNavServices}
        >
          Servicios
        </NavLink>
        {windowWidth > 1000 ? (
          menuServices ? (
            <ul className="menu-container-li-ul">
              {linkServices.map((s) => (
                <li className="menu-container-li-ul-li" key={s.id}>
                  <NavLink to={s.url}>{s.nombre}</NavLink>
                </li>
              ))}
            </ul>
          ) : (
